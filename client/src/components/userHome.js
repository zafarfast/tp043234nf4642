...
export default function UserHome() {
    ...
    mutation Mutation($username: String!, $thoughtText: String!, $imageUrl: String!) {
        addPost(username: $username, thoughtText: $thoughtText, imageUrl: $imageUrl) {
          username
          thoughtText
          imageUrl
        }
    ...
    const [generatedImageContainer, setgeneratedImageContainer] = useState('display-none')
    ...
    const [submitPost] = useMutation(POST_THOUGHT, {
        variables: {
            ...
            imageUrl: "https://external-content.duckduckgo.com/iu/?u=https%3A%2F%2Fi.pinimg.com%2Foriginals%2Fbe%2Fce%2F0c%2Fbece0c797cb134aefb2cb836578c9249.gif&f=1&nofb=1&ipt=4ccc34333fe26eae9a014c878fa2c94a57a2c15bf377b7286897aea8e3ad4892&ipo=images"
        }
    });
    ...
    const [renderImageResponse, setrenderImageResponse] = useState({ data: { getUserThought: './images/travelling the world alone.jpg' } });
        ...
        for (let i = findUsers.length - 1; i >= 0; i--) {
            if (findUsers[i].posts) {
                for (let j = findUsers[i].posts.length - 1; j >= 0; j--) {
                    posts.push({ ...findUsers[i].posts[j], displayPicture: findUsers[i].displayPicture })
                }
            }
        }
    }
    ...
    return <>
        ...
        <div id="search-form-container">
            <form id="search-form" onSubmit={handleSubmit}>
                <input name="thought" value={formData.thought} onChange={handleChange} id="search-field" placeholder="Tell us what you are thinking today." type="text"></input>
        ...
        <div className={generatedImageContainer}>
            <div id="generated-image-container">
                ...
                <div>
                    ...
                    <button onClick={handlePost} id="post-button">Post </button>
                    <button onClick={handleSubmit} id="re-generate-button">Generate New Image </button>
                </div>
            </div>
        </div>
